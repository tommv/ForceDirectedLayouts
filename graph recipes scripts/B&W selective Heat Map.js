...
settings.offset = 50 // Margin

// Heatmap
settings.spreading = 140.0 // Pixel diameter of each nodes' "heat" area
settings.quantize = true // Disable for a continuous scale
settings.quantization_colors = 16

// --- (end of settings)

g.nodes().forEach(function(nid){
  n = g.getNodeAttributes(nid)
  n.heatmapScore = 1                            // SET TO ZERO TO WEIGHT THE HEATMAP BY NODE SIZE
  //if (n['Language'] == 'English') {           // USE THIS LINE TO DESIGN THE HEATMAP OF ONE PARTITION ONLY
  //      n.heatmapScore = n.size*1                 // USE THIS LINE TO WEIGHT THE HEATMAP BY NODE SIZE
  // }                                          // USE THIS LINE TO DESIGN THE HEATMAP OF ONE PARTITION ONLY
})
  ...
  for (x = Math.max(0, Math.floor(n.x - settings.spreading/2) ); x <= Math.min(settings.width, Math.floor(n.x + settings.spreading/2) ); x++ ){
    for (y = Math.max(0, Math.floor(n.y - settings.spreading/2) ); y <= Math.min(settings.height, Math.floor(n.y + settings.spreading/2) ); y++ ){
      ...
      if (d < settings.spreading / 2) {
        ...
        value = n.heatmapScore * (1 - 2 * d / settings.spreading)
...
for ( i = 0, pixlen = pix.length; i < pixlen; i += 4 ) {
  ...
  var color = d3.rgb(d3.interpolate('#000', '#FFF')(value))
  pix[i  ] = color.r // red
